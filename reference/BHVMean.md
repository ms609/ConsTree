...
to the sample (Brown and Owen 2020) . The mean is approximated by the
iterative law-of-large-numbers algorithm of Sturm (2003) and Miller et
al. (2015) : starting from a sample tree, each step walks
\\\frac{1}{k+1}\\ of the way along the geodesic towards a randomly
chosen sample tree.
...
- tolerance:
  ...
  Numeric stating convergence threshold relative to the sample standard
...
`BHVMean()` returns an object of class `phylo` denoting the mean tree,
with attributes `iterations` (integer; number of steps taken) and
`converged` (logical). Because the step length shrinks as \\1/(k+1)\\,
`converged = TRUE` indicates that successive estimates have stopped
moving appreciably (the stopping rule was met before `maxIter`), not a
guaranteed bound on the distance to the exact mean; tighten `tolerance`
...
squared geodesic distance from the sample to its mean,
\\\frac{1}{r}\sum_i d(\bar T, T_i)^2\\; with `type = "sum"`, the total
\\\sum_i d(\bar T, T_i)^2\\.
