...
- If both inputs are single trees, the return value is a single
  non-negative number.

- When comparing a tree to a collection of trees, the return value is a
  named numeric vector with one entry per tree in the collection.

- If `tree2` is omitted, or is identical to `tree1`, the return value is
  a [`stats::dist`](https://rdrr.io/r/stats/dist.html) object of all
  pairwise distances.

- If `tree1` and `tree2` are different collections, the return value
  comprises a numeric matrix with rows corresponding to `tree1` and
  columns to `tree2`.
