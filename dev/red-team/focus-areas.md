...
| 9 | **Transfer consensus** | `src/transfer_consensus.cpp`, `R/transfer.R` | sonnet | **REVIEWED 2026-06-15** (Rounds 1–3, sonnet). TC-001/002/004/005/006/007 fixed (e4e0aa9); TC-008/009 cross-TU ODR stack-overflow fixed via anonymous namespaces (Round 3, ASAN-driven). Seam still yielding → stays sonnet. Next revisit: greedy-heuristic optimality (untested). |
| 10 | **FACT primitive & C++ memory safety** | `src/fact_tree.cpp/.h`, cross-cutting `src/*.cpp` bounds, OpenMP | sonnet | **REVIEWED 2026-06-15 (Round 4, sonnet).** Fixed: cross-TU ODR (TC-009) + `M*M` overflow in Quartet `compat` (MEM-001, twin of TC-004). C-001/C-004 **refuted**. Confirmed latent/degenerate (unreachable via R API): C-006 dropped `assert(num==N)`, C-007 dead root-fallback, C-002 `lcaDepth` past root −1, C-003 `goodLabel`. New LOW: MEM-002 `bool*`/`char*` aliasing in `cons_frequency.cpp`. ODR sweep + OpenMP clean. Seam yielding → stays sonnet. Next: large-M regression test; C-002/3/6/7 defensive guards for CRAN UBSan. |
...
## Notes
- **Area 9 (Transfer) reviewed 2026-06-15** (Rounds 1–3); the ASAN-driven Round 3
  found a cross-TU ODR memory-corruption bug (TC-009) shared with Quartet.
  `last_focus` is 9 (see `log.md`); a bare `/red-team` now picks **area 10**.
...
- **Cross-TU ODR is now a standing area-10 check:** no type/inline/global may be
  defined at namespace scope under the same name in two `.cpp` files. The
  `cons_*.cpp` files use anonymous namespaces; Quartet/transfer now do too.
